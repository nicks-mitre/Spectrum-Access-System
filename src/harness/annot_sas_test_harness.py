...
class SasTestHarnessServer(threading.Thread):
	...
	def __init__(
		...
		Args:
			name: A name for SAS Test Harness.If multiple SAS Test Harnesses are created then this value should be unique.
		...
		self.dump_path: str = self.generateTempDirectory()
		self.name: str = name
		self.host_name: str = host_name
		self.port: int = port
		self.sas_version: str = self.getSasTestHarnessVersion()

		self.base_url = host_name + ':' + str(port)
		# BaseURL format should be https: // < hostname >: < port > / versionX.Y
		self.http_server_url: str = 'https://' + host_name + ':' + str(port) + '/' + self.sas_version
		self.cert_file: str = cert_file if cert_file is not None else DEFAULT_CERT_FILE
		self.key_file: str = key_file if key_file is not None else DEFAULT_KEY_FILE
		self.ca_cert_file: str = ca_cert_file if ca_cert_file is not None else DEFAULT_CA_CERT
		self.setDaemon(True)
	...
	def generateTempDirectory(self) -> str:
		"""
	...
	def getSasTestHarnessVersion(self) -> str:
		"""
		This method retrieves the SAS Test Harness Version from sas.cfg.
		Returns: Returns SAS version of the SAS Test Harness to be used to construct the base URL.
		"""
	...
	def __writeDumpFile(self, file_name: str, data: Dict):
	...
	def writeFadRecords(
		...
		full_activity_dump = self.__createFadObject(fad_record_list, fad_generation_time)
...
def generateCbsdReferenceId(fcc_id, serial_number) -> str:
	"""
...
def generateCbsdRecords(
	registration_requests: List, # CBSD Reg Requests list
	grant_requests_list: List # CBSD Grant Requests list
) -> List[CbsdData]:
	"""
	...
	Args:
		...
		grant_requests_list: List of muliple grant requests for all cbsd devices.
				For an example : muliple grants [ [grant1, grant2], [grant3, grant4 ] are associated with each cbsd records [ cbsd1, cbsd2 ].
	Returns:
...
def generatePpaRecords(
	ppa_records: List[Ppa_Record],
	cbsd_reference_ids: List[List]
) -> List[Ppa_Record]:
	"""
