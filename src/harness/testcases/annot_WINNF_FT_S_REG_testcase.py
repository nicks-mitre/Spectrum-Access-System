...
class RegistrationTestcase(sas_testcase.SasTestCase):
  ...
  def test_WINNF_FT_S_REG_6(self):
	...
	Release 1 Instructions:
		Ensure that the DP Test Harness sends a Registration Request in the form of one 3-element Array to the SAS [is] as follows:
			- valid userId, fccId, and cbsdSerialNumber plus all REG-Conditional parameters for Cat A CBSD
			shall be included for the first and second CBSDs (Cat A), except that for the second CBSD at
			least one REG-Conditional parameter for a Cat A CBSD missing.
			- valid userId, fccId, and cbsdSerialNumber shall be included for the third CBSD (Cat B). 
			The encodedCpiSignedData parameter in the cpiSignatureData parameter shall be set to the value 
			generated by using the CpiSignedData object that contains installationParam parameter with all 
			the required parameters being certified by the CPI, except for one REG-Condition al parameter.
		
	Release 2 Instructions:
		The Test Harness sends a Registration Request for the CBSD to the SAS UUT	
			- The Registration Request is in proper format and parameters are within acceptable ranges. 	
			- cbsdFeatureCapabilityList is included with FID: WF_GRANT_UPDATE
    """
